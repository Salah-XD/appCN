...
pnpm --filter @app-cn/web dev      # web (landing + docs) only
...
pnpm lint
pnpm registry:build               # clean + `shadcn build` + import-rewrite + validate-registry.mjs
pnpm changeset                    # describe what changed (required for any packages/* edit)
pnpm changeset status             # check whether a changeset is pending
```
...
Every component ships **eight** artifacts. The PR template enforces them; reviewers bounce PRs
...
8. Changeset committed (`pnpm changeset` → `@app-cn/ui` minor). The release pipeline gates
   on this; the Changesets bot will flag PRs that skip it.

## Distribution

appCN ships **three** ways — same primitives, consumer's choice:

- **`appcn` CLI** (recommended): `npx @app-cn/cli@latest init` then
  `npx @app-cn/cli@latest add <slug>`. The CLI handles NativeWind +
  Reanimated config and registers `"@app-cn"` in `components.json`, then
  delegates to `shadcn@latest add @app-cn/<slug>` under the hood. Source in
  `packages/cli/`, docs at `/docs/cli` on the live site.
- **shadcn CLI** (copy-paste, no lock-in):
  `npx shadcn@latest add https://appcn.vercel.app/r/<slug>.json` or the namespaced form
  `npx shadcn@latest add @app-cn/<slug>` (after registering `"@app-cn"` in `components.json`).
  Served by `apps/web/public/r/*.json`, regenerated by `pnpm registry:build` on every deploy.
- **npm** (managed dep): `npm install @app-cn/ui`. Ships TS source only (`files: ["src",
  "tailwind.preset.js", "README.md", "LICENSE", "CHANGELOG.md"]`); Metro / Next / Vite transpile
  TS themselves — no build step.

**Publishing:** Changesets-driven via `.github/workflows/release.yml`. Push to
`main` either opens a "Version Packages" PR (if pending `.changeset/*.md` files
exist) or publishes anything ahead of npm via OIDC Trusted Publishing.
`workflow_dispatch` is now a pack-only dry-run (no real publish). `registry-check.yml`
runs the validator on every PR. The docs site (`/components/<slug>` → Install tabs,
`/docs/cli`, landing hero pill) surfaces all three install paths.
...
- Cross-references between items use the **namespaced** form (`registryDependencies: ["@app-cn/cn"]`),
  installed via a consumer `components.json` registry entry `"@app-cn": ".../r/{name}.json"`.
...
- Verified install loop: `npx shadcn@latest add @app-cn/button` pulls the component + its `cn`
...
- **Metro `watchFolders` + `nodeModulesPaths`** (see `apps/showcase/metro.config.js`) include the
  workspace root so the Expo app resolves `@app-cn/ui` and hoisted deps; `disableHierarchicalLookup`
  is on.
